...
class BGPnode:
	...
	'''
	Returns all candidate best paths (i.e. values in the "all_paths" dictionary) for the given prefix

	Input argument:
		(a) IPprefix:	the prefix the path to which is requested

	Returns:
		A list of paths, i.e., a list of lists of ASNs, for the given prefix
	'''

	def get_all_paths(self, IPprefix):
		all_paths_dict = self.all_paths.get(IPprefix)
		if all_paths_dict is not None:
			return list(self.all_paths.get(IPprefix).values())
		print("WRONG")
		return []
	...
	def conditions_to_change_existing_path(self,IPprefix, new_path):
		...
		if self.ASneighbors[new_path[0]] < self.ASneighbors[my_path[0]]:	# Prefer customer (-1) > peer (0) > provider (1)
			...
			if len(new_path) < len(my_path):	# if new path is shorter ...prefer it
				...
				return new_path[0] < my_path[0]	# ... if new neighbor has lowest ASn prefer it
